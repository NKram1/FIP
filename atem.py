...
if __name__ == '__main__':
    ...
    a.connectToSwitcher ((config.address,9910))
        #a.waitForPacket()
        #a.sendCommand("CPgI" ,"\x00\x05\x00\x00")
...
##        else:
##            num += 1
        numval = ('0'+str(num)).decode('hex')
        #a.waitForPacket()
        a.waitForPacket()
        a.sendCommand ("CPgI", "\x00"+numval+"x00\x00")
        #a.waitForPacket()
        #print a.isInitialized
        #x = raw_input("Press any key")
        time.sleep(.1)
        #break
        #a.sendCommand ("CPgI", "\x00\x02\x00\x00")
